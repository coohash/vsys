...
├── vsys_supernode_install.txt             # VSYS区块链节点安装（包含主网数据快照等）
...
├── utils/                       # 底层公共组件包
│   ├── __init__.py              # 模块初始化
│   ├── crypto_probe.py          # 底包兼容适配器：动态探测全版本库签名函数 ['sign', 'sign_data', 'signature']
│   └── tx_builder.py            # 底层手动字节流拼装器：抛弃高层依赖，严格基于 struct.pack 拼装大端序字节
│
└── data/                        # 数据层定义
    ├── list_address.csv         # 输入文件示例：待处理的钱包地址列表（每行一个标准地址）
    ├── balance_address.csv      # 输出文件示例：高并发探测后自动生成的地址余额结果报表
    └── vsys_snapshot.json       # 历史快照数据：链上钱包地址余额全量快照
